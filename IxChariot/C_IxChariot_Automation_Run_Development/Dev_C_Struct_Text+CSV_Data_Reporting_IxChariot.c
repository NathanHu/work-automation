 ...
 * Made as a function that can be implemented in other scripts
 */
void get_file_name( char *name_var, char *csv, char *txt, Filename info )
{
...
* Made as a function that can be implemented in other scripts
*/
void set_row_header( char *header, Header info ) {

	memset(header, 0, 30);
	snprintf( header, 30, "%s-%s-%s-%d", info.client, info.location, info.wifi_channel, info.streams );
	printf( "Row identifier is %s\n", header );
  ...
  **************************************************************************/
	 ...
	 */
	 ...
	 /* *** 3.2 Write to the .txt file (currently manual) *** */
	  /* *** DATASET 1 *** */
	printf("********* DATASET 1 ********\n");
	Header header = { client_id[0], &location, &wifi_channel, num_streams };
	set_row_header(row_header, header);		// FOR TEST. ADD TO LOOP IN REAL CODE.
